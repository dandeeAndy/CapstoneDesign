...
bool CustomCalibration::Initialize()
{
	...
	bool ret = m_rsDevice.InitializeCamera("");
	if (!ret) return false;

	m_cameraInfo = m_rsDevice.Get_Active_CameraInfo();
    ...
    string pid = m_rsDevice.GetDevicePID();

    // Intel RealSense Depth Camera D455
    if (pid.compare("0B5C") == 0)
    {
        m_rgbWidth = 1280;
        m_rgbHeight = 800;
    }
...
void CustomCalibration::Start(void)
{
	...
	m_rsDevice.EnableAutoExposure(1.0f, false);
	if (isWide())
		m_rsDevice.SetAeControl(800);
	else
		m_rsDevice.SetAeControl(500);

	m_rsDevice.SetMediaMode(m_width, m_height, m_fps, m_rgbWidth, m_rgbHeight, m_fps, false, true, supportsRGB(), rs2_format::RS2_FORMAT_Y16);

	m_rsDevice.StartCapture([&](const void *leftImage, const void *rightImage,
		const void *colorImage, const void *depthImage, const uint64_t timeStamp)
	{
        ...
        // On Intel RealSense Depth Camera D455, left/right sensor colored with bayer pattern, need to debayr first
		if ((isWide()) && pid.compare("0B5C") != 0)
		{
		...
		if (m_imagesCapture && m_numShot < NUM_SHOTS)
		{
			...
			if (rt)
			{
				...
				if (rt && supportsRGB() && colorImage != NULL)
				{
...
void CustomCalibration::OnDisplay(void)
{
	...
	bool busy = m_inProcessing || m_inUpdating || m_shotFailed;
	...
	if (supportsRGB())
	{
		...
		uint8_t *colorImg = (uint8_t*)m_colorImage.get();
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_rgbWidth, m_rgbHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, colorImg);
	...
	else
	{
		...
		glTexCoord2d(0, 1); glVertex2d(-1.0, -h/2.0);
		glTexCoord2d(0, 0); glVertex2d(-1.0, h/2.0);
		glTexCoord2d(1, 0); glVertex2d(0.0, h/2.0);
		glTexCoord2d(1, 1); glVertex2d(0.0, -h/2.0);
		glEnd();
		...
		glTexCoord2d(0, 1); glVertex2d(0.0, -h/2.0);
		glTexCoord2d(0, 0); glVertex2d(0.0, h/2.0);
		glTexCoord2d(1, 0); glVertex2d(1.0, h/2.0);
		glTexCoord2d(1, 1); glVertex2d(1.0, -h/2.0);
		glEnd();
		glDisable(GL_TEXTURE_2D);
