...
namespace CustomCalibrator
{
	...
	const float CHESSBOARD_SQUARE_SIZE = 60.0f;  // change to 30.0f if target printed on regular letter size 8.5" x 11" printer paper
	...
	private:
        ...
        bool supportsRGB() {
            ...
            if (pid.compare("0B5B") == 0)
                ...
                return m_rsDevice.SupportsRGB();
        }

        bool isWide() {
            return m_cameraInfo.features & SKU_WIDE;
        }

	private:
		Rs400Device m_rsDevice;
