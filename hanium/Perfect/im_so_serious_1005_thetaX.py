...
def process_roi(color_image, depth_image, roi):
    ...
    box, center = find_rectangle(color_roi)
    ...
    if box is not None and center is not None:
        ...
        return qr_data, global_center, box, floor, depth
...
def process_frame(frameset, rois):
    ...
    if find_rect:
        rect_info = []
        for i, roi in enumerate(rois):
            _, center, _, floor, depth = process_roi(color_image, depth_image, roi)
        ...
        if rect_info:
            ...
            qr_data, center, box, floor, depth = process_roi(color_image, depth_image, selected_roi)
            
            if center is not None:
                current_location = (floor, current_roi_index + 1)
...
def motor_move(user_option, data_queue_QR):
    ...
    try:
        for position in pick_positions:
            ...
            solenoid.airpump_on()
            time.sleep(1)
            motor_set_4.move(position)  # theta 제거
            time.sleep(1)
