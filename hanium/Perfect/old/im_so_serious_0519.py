...
def motor_move(option, data_queue_QR):
    global current_step_A, current_step_B, parts, position, Motor_data
    ...
    if not data_queue_QR.empty():
        ...
        if motor_key:
            ...
            position = getattr(motor_set_4, motor_key)[current_step % len(getattr(motor_set_4, motor_key))]
            position_angle = motor_set_4.place_position[position]
            motor_set_4.move_to_position(position_angle)
