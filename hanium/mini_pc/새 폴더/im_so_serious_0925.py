...
LONG_RANGE = (160, 250)  # 긴 변의 범위 (픽셀)
X_OFFSET = -24.6  # 좌표계 변환을 위한 오프셋
Y_OFFSET = 6.4
pixel_to_mm = 0.06875  # 픽셀 당 mm 비율
CONSISTENT_FRAMES = 5
...
def pixel_to_robot_coordinates(x, y, z):
    """픽셀 좌표를 로봇 좌표계로 변환"""
    robot_x = (x * pixel_to_mm) + X_OFFSET
    robot_y = (y * pixel_to_mm) + Y_OFFSET
    robot_z = z  # z 좌표(깊이)는 이미 mm 단위일 것으로 가정
    ...
    if find_rect:
        ...
        if rect_info:
            ...
            if center is not None:
                ...
                pixel_z = depth
                robot_x, robot_y, robot_z = pixel_to_robot_coordinates(pixel_x, pixel_y, pixel_z)
...
def calculate_motor_angles(x, y, z, t):
    ...
    angle_A = robot_move.deltakinematic('A') - 12.253
    angle_B = robot_move.deltakinematic('B') - 12.253
    angle_C = robot_move.deltakinematic('C') - 12.253
