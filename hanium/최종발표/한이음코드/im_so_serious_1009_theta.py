...
# Constants
DEPTH_RANGES = [(300, 340), (360, 390), (400, 450)]  # 3층, 2층, 1층 
SHORT_RANGE = (100, 200) 
LONG_RANGE = (100, 200)
CONSISTENT_FRAMES_ROI = 30
CONSISTENT_FRAMES_THETA = 10
...
def process_frame(frameset, rois):
    ...
    if find_rect:
        ...
        for i, roi in enumerate(rois):
            ...
            if center is not None and floor is not None:
                ...
                cv2.putText(color_image, f"Floor: {floor}, Depth: {depth}", 
    ...
    for i, roi in enumerate(rois):
        ...
        if i == current_roi_index:
            cv2.rectangle(overlay, (roi['x'], roi['y']), (roi['x'] + roi['w'], roi['y'] + roi['h']), (0, 0, 255), -1)
            cv2.addWeighted(overlay, 0.3, color_image, 0.7, 0, color_image)
            cv2.rectangle(color_image, (roi['x'], roi['y']), (roi['x'] + roi['w'], roi['y'] + roi['h']), (0, 0, 255), 2)
        else:
            cv2.rectangle(color_image, (roi['x'], roi['y']), (roi['x'] + roi['w'], roi['y'] + roi['h']), (255, 0, 0), 2)
        
        cv2.putText(color_image, f"ROI {i+1}", (roi['x']+10, roi['y']+30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Depth colormap에도 동일하게 적용
        if i == current_roi_index:
            cv2.rectangle(depth_colormap, (roi['x'], roi['y']), (roi['x'] + roi['w'], roi['y'] + roi['h']), (0, 0, 255), 2)
        else:
            cv2.rectangle(depth_colormap, (roi['x'], roi['y']), (roi['x'] + roi['w'], roi['y'] + roi['h']), (255, 0, 0), 2)
        cv2.putText(depth_colormap, f"ROI {i+1}", (roi['x']+10, roi['y']+30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
def read_qr_code(client_socket):
    global QR_data, Vision_start_signal, Motor_start_signal, parts, current_roi_index, last_qr_data, qr_data_list, location, SEND_QR_data
            ...
            if new_qr_data and new_box_info:
                ...
                with lock:
                    if new_qr_data != last_qr_data:
                        ...
                        SEND_QR_data = f"{new_qr_data}/{'Vision'}/{'STR'}/{recognition_time}"
                        ...
                        try:
                            time.sleep(1)
                            print(f"Sent QR Data: {SEND_QR_data}")
                        except Exception as e:
                            print(f"Error sending QR data: xxxxxc{e}")
                            Vision_start_signal = False
                            break
...
def motor_move(user_option, data_queue_QR):
    global current_step_A, current_step_B, parts, place_position, Motor_data, location
    ...
    option_map = {  'Option1': {'A': 'motor_positions_A', 'B': 'motor_positions_B'},
...
def server_func():
    global QR_data, Motor_data, Option_select, data_queue_QR,SEND_QR_data
    ...
    try:
        while True:
            if Option_select:
                if SEND_QR_data and SEND_QR_data != last_sent_QR_data:
                    client_soc.sendall(SEND_QR_data.encode('utf-8'))
                    last_sent_QR_data = SEND_QR_data
                    time.sleep(1)
            ...
            if data.startswith("Option"):
                ...
                if qr_data_list:
                    try:
                        ...
                        save_path = 'C:/Users/Lee/Desktop/Perfect/QR_Data/qr_data.xlsx'
